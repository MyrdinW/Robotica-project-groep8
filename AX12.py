...
class Ax12:
    ...
    def moveInWheelMode(self, id, speed):
        self.direction(Ax12.RPI_DIRECTION_TX)
        Ax12.port.flushInput()
        p = [speed&0xff, speed>>8]
        checksum = (~(id + Ax12.AX_GOAL_LENGTH + Ax12.AX_WRITE_DATA + Ax12.AX_GOAL_SPEED_L + p[0] + p[1]))&0xff
        outData = bytes([Ax12.AX_START])
        outData += bytes([Ax12.AX_START])
        outData += bytes([id])
        outData += bytes([Ax12.AX_GOAL_LENGTH])
        outData += bytes([Ax12.AX_WRITE_DATA])
        outData += bytes([Ax12.AX_GOAL_SPEED_L])
        outData += bytes([p[0]])
        outData += bytes([p[1]])
        outData += bytes([checksum])
        Ax12.port.write(outData)
